...
import io.horizontalsystems.oxyrakit.model.TransactionInfo.Direction
...
class OxyraTransactionsAdapter(
    ...
    override fun getTransactionUrl(transactionHash: String): String =
        "https://explorer.eer-wsd.com/tx/$transactionHash"
    ...
    private fun getTransactionRecord(transaction: TransactionInfo): TransactionRecord {
        ...
        return when (transaction.direction) {
            Direction.Direction_In -> {
                ...
                OxyraIncomingTransactionRecord(
                    token = wallet.token,
            ...
            Direction.Direction_Out -> {
                OxyraOutgoingTransactionRecord(
                    token = wallet.token,
                    uid = transaction.hash,
                    transactionHash = transaction.hash,
                    transactionIndex = 0,
                    ...
                    to = transaction.transfers?.firstOrNull()?.address,
                    sentToSelf = false,
                    memo = transaction.notes,
                    source = wallet.transactionSource,
