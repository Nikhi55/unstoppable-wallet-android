...
import io.horizontalsystems.bankwallet.modules.send.oxyra.SendOxyraConfirmationScreen
...
class SendConfirmationFragment : BaseComposeFragment() {
    ...
    override fun GetContent(navController: NavController) {
        withInput<Input>(navController) { input ->
            when (input.type) {
                ...
                Type.Oxyra -> {
                    val sendOxyraViewModel by navGraphViewModels<SendOxyraViewModel>(R.id.sendXFragment)

                    SendOxyraConfirmationScreen(
                        navController,
                        sendOxyraViewModel,
                        input.sendEntryPointDestId
                    )
    ...
    enum class Type : Parcelable {
        Bitcoin, ZCash, Solana, Tron, Ton, Stellar, Monero, Oxyra
    }
