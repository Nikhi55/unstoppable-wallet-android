...
import io.horizontalsystems.bankwallet.modules.manageaccount.showoxyrakey.ShowOxyraKeyModule.OxyraKeyType
...
class ShowOxyraKeyFragment : BaseComposeFragment(screenshotEnabled = false) {
    ...
    override fun GetContent(navController: NavController) {
        ...
        if (keys == null) {
            ...
            ShowOxyraKeyScreen(navController, keys)
        }
    }

    @Parcelize
    data class Input(val keys: OxyraKeys) : Parcelable
}

@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun ShowOxyraKeyScreen(
    ...
    var keyType by remember { mutableStateOf(OxyraKeyType.Spend) }
        ...
        Column {
            Column(
                ...
                CellUniversalLawrenceSection {
                    MenuItem(
                        title = stringResource(R.string.OxyraKeyType),
                ...
                if (showKeyTypeSelectorDialog) {
                    MenuGroup(
                        title = stringResource(R.string.OxyraKeyType),
                        items = OxyraKeyType.entries.map {
