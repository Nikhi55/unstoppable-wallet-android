...
package io.horizontalsystems.oxyrakit.model;
...
public class Wallet {
    ...
    static public class Status {
        ...
        public boolean isOk() {
            ...
            boolean connectionOk = (getConnectionStatus() == null) ||
                    (getConnectionStatus() == ConnectionStatus.ConnectionStatus_Connected) ||
                    (getConnectionStatus() == ConnectionStatus.ConnectionStatus_WrongVersion);
            return statusOk && connectionOk;
        }
    ...
    public ConnectionStatus getConnectionStatus() {
        try (ExecutorService executor = Executors.newSingleThreadExecutor()) {
            Future<ConnectionStatus> future = executor.submit(() -> {
                ...
                ConnectionStatus status = ConnectionStatus.values()[s];
                // Oxyra daemon uses only 1 hard fork (v1); the wallet's compiled
                // hard fork table has 2 entries (v1 + v12) which triggers a false
                // WrongVersion from the C++ library. Developer confirmed 1 is correct.
                if (status == ConnectionStatus.ConnectionStatus_WrongVersion) {
