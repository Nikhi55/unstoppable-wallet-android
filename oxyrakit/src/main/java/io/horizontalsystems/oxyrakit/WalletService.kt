package io.horizontalsystems.oxyrakit

import android.content.Context
import android.util.Log
import io.horizontalsystems.oxyrakit.data.TxData
import io.horizontalsystems.oxyrakit.model.PendingTransaction
import io.horizontalsystems.oxyrakit.model.TransactionInfo
import io.horizontalsystems.oxyrakit.model.Wallet
import io.horizontalsystems.oxyrakit.model.Wallet.Status
import io.horizontalsystems.oxyrakit.model.WalletListener
import io.horizontalsystems.oxyrakit.model.WalletManager
import io.horizontalsystems.oxyrakit.util.Helper
import io.horizontalsystems.oxyrakit.util.NetCipherHelper
...
class WalletService(private val context: Context) {
    ...
    fun start(wallet: Wallet, trustNode: Boolean): Status? {
        ...
        Log.e("eee", "WS.start() address=${wallet.address} restoreHeight=${wallet.restoreHeight} blockchainHeight=${wallet.blockChainHeight}")

        initWallet(wallet, trustNode)

        val walletStatus = wallet.fullStatus
        Log.e("eee", "WS.start() after init: status=${walletStatus} connectionStatus=${walletStatus.connectionStatus} errorString='${walletStatus.errorString}' daemonHeight=${wallet.daemonBlockChainHeight} walletHeight=${wallet.blockChainHeight}")

        if (!walletStatus.isOk) {
            Log.e("eee", "WS.start() status NOT OK, stopping. isOk=${walletStatus.isOk}")
            stop()
            return walletStatus
        }
    ...
    private inner class MyWalletListener : WalletListener {
        ...
        override fun moneySpent(txId: String, amount: Long) { Log.e("eee", "moneySpent() amount=$amount txId=$txId") }
        override fun moneyReceived(txId: String, amount: Long) { Log.e("eee", "moneyReceived() amount=$amount txId=$txId") }
        override fun unconfirmedMoneyReceived(txId: String, amount: Long) { Log.e("eee", "unconfirmedMoneyReceived() amount=$amount txId=$txId") }
        ...
        override fun newBlock(height: Long) {
            ...
            if (lastBlockTime < System.currentTimeMillis() - 2000) {
                ...
                Log.d("eee", "newBlock($height) isSynchronized=${wallet.isSynchronized} daemonHeight=$daemonHeight walletHeight=${wallet.blockChainHeight} balance=${wallet.balance}")
                if (observer != null) {
        ...
        override fun refreshed() {
            ...
            Log.e("eee", "refreshed() status=$walletFullStatus connectionStatus=${walletFullStatus.connectionStatus} errorString='${walletFullStatus.errorString}' walletHeight=${wallet.blockChainHeight} daemonHeight=${wallet.daemonBlockChainHeight} balance=${wallet.balance} unlockedBalance=${wallet.unlockedBalance} updated=$updated isSynchronized=${wallet.isSynchronized}")

            if (!walletFullStatus.isOk) {
                Log.e("eee", "refreshed() status NOT OK: isOk=${walletFullStatus.isOk} status=${walletFullStatus.status} connectionStatus=${walletFullStatus.connectionStatus} error='${walletFullStatus.errorString}'")
                observer!!.onRefreshed(wallet, walletFullStatus, false)
                return
            }

            val dHeight = wallet.daemonBlockChainHeight
            val wHeight = wallet.blockChainHeight
            if (dHeight > 0 && wHeight >= dHeight) {
                Log.e("eee", "refreshed() calling setSynchronized() walletH=$wHeight >= daemonH=$dHeight")
                wallet.setSynchronized()
            } else {
                Log.e("eee", "refreshed() NOT setting synchronized: walletH=$wHeight daemonH=$dHeight")
            }
            if (updated) {
                updateDaemonState(wallet, wallet.blockChainHeight)
                wallet.refreshHistory()
                val txCount = wallet.history.all?.size ?: 0
                Log.e("eee", "refreshed() after refreshHistory: txCount=$txCount daemonHeight=$daemonHeight")
                if (observer != null) {
